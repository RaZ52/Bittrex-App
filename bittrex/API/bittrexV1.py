...
class Bittrex(object):
    ...
    def make_auth_request(self, uri, params=None):
        ...
        if params is None:
            ...
            uri += '?apikey=%s' % self.api_key
            params.update({'nonce': nonce})
            print(params)
        uri = Request('GET', uri, params=params).prepare()
        uri = uri.url
        api_sign = hmac.new(self.api_secret.encode('ASCII'), uri.encode('ASCII'), hashlib.sha512).hexdigest()
    ...
    def get_open_orders(self, market=None):
        ...
        uri = MARKET_BASEURL + '/getopenorders'
        if market is not None:
